...
import com.ruoyi.isup.service.AlarmService.HCISUPAlarm;
...
public class Stream {
    ...
    public static boolean stopPlayBackFlag = false;  //用来标记是否回放结束
    static int Count = 0;
    static int iCount = 0;
    ...
    public void startRealPlayListen_Win() {
        ...
        System.arraycopy(isupConfig.getIp().getBytes(), 0, struPreviewListen.struIPAdress.szIP, 0, isupConfig.getIp().length());
        struPreviewListen.struIPAdress.wPort = Short.parseShort(String.valueOf(isupConfig.getSmsServerListenPort())); //流媒体服务器监听端口
    ...
    public void startPlayBackListen_WIN() {
        ...
        System.arraycopy(isupConfig.getIp().getBytes(), 0, struPlayBackListen.struIPAdress.szIP, 0, isupConfig.getIp().length());
        struPlayBackListen.struIPAdress.wPort = Short.parseShort(isupConfig.getSmsBackServerListenPort()); //流媒体服务器监听端口
    ...
    public void RealPlay(int lChannel) {
        ...
        struPreviewIn.struStreamSever.wPort = Short.parseShort(String.valueOf(isupConfig.getSmsServerPort())); //流媒体服务器端口，需要跟服务器启动监听端口一致
        struPreviewIn.write();
        //预览请求
        HCISUPCMS.NET_EHOME_PREVIEWINFO_OUT struPreviewOut = new HCISUPCMS.NET_EHOME_PREVIEWINFO_OUT();
        boolean getRS = CMS.hCEhomeCMS.NET_ECMS_StartGetRealStream(1, struPreviewIn, struPreviewOut);
        //Thread.sleep(10000);
        if (!CMS.hCEhomeCMS.NET_ECMS_StartGetRealStream(1, struPreviewIn, struPreviewOut)) {
            System.out.println("NET_ECMS_StartGetRealStream failed, error code:" + CMS.hCEhomeCMS.NET_ECMS_GetLastError());
            return;
        } else {
            struPreviewOut.read();
        ...
        if (!CMS.hCEhomeCMS.NET_ECMS_StartPushRealStream(1, struPushInfoIn, struPushInfoOut)) {
            System.out.println("NET_ECMS_StartPushRealStream failed, error code:" + CMS.hCEhomeCMS.NET_ECMS_GetLastError());
            return;
        } else {
    ...
    public void StopRealPlay() {
        ...
        if (!CMS.hCEhomeCMS.NET_ECMS_StopGetRealStream(1, sessionID)) {
            System.out.println("NET_ECMS_StopGetRealStream failed,err = " + CMS.hCEhomeCMS.NET_ECMS_GetLastError());
            return;
        }
    ...
    public void PlayBackByTime(int lchannel) {
        ...
        m_struPlayBackInfoIn.unionPlayBackMode.struPlayBackbyTime.struStartTime.wYear = 2024;
        m_struPlayBackInfoIn.unionPlayBackMode.struPlayBackbyTime.struStartTime.byMonth = 9;
        m_struPlayBackInfoIn.unionPlayBackMode.struPlayBackbyTime.struStartTime.byDay = 11;
        m_struPlayBackInfoIn.unionPlayBackMode.struPlayBackbyTime.struStartTime.byHour = 0;
        ...
        m_struPlayBackInfoIn.unionPlayBackMode.struPlayBackbyTime.struStopTime.wYear = 2024;
        m_struPlayBackInfoIn.unionPlayBackMode.struPlayBackbyTime.struStopTime.byMonth = 9;
        m_struPlayBackInfoIn.unionPlayBackMode.struPlayBackbyTime.struStopTime.byDay = 11;
        m_struPlayBackInfoIn.unionPlayBackMode.struPlayBackbyTime.struStopTime.byHour = 0;
        ...
        System.arraycopy(isupConfig.getIp().getBytes(), 0, m_struPlayBackInfoIn.struStreamSever.szIP,
                0, isupConfig.getIp().length());
        m_struPlayBackInfoIn.struStreamSever.wPort = Short.parseShort(isupConfig.getSmsBackServerListenPort());
        ...
        if (!CMS.hCEhomeCMS.NET_ECMS_StartPlayBack(1, m_struPlayBackInfoIn, m_struPlayBackInfoOut)) {
            System.out.println("NET_ECMS_StartPlayBack failed, error code:" + CMS.hCEhomeCMS.NET_ECMS_GetLastError());
            return;
        } else {
        ...
        if (!CMS.hCEhomeCMS.NET_ECMS_StartPushPlayBack(1, m_struPushPlayBackIn, m_struPushPlayBackOut)) {
            System.out.println("NET_ECMS_StartPushPlayBack failed, error code:" + CMS.hCEhomeCMS.NET_ECMS_GetLastError());
            return;
        } else {
            System.out.println("NET_ECMS_StartPushPlayBack succeed, sessionID:" + m_struPushPlayBackIn.lSessionID + ",lUserID:" + 1);
        }
    }
    ...
    public void stopPlayBackByTime() {
        if (!CMS.hCEhomeCMS.NET_ECMS_StopPlayBack(1, backSessionID)) {
            System.out.println("NET_ECMS_StopPlayBack failed,err = " + CMS.hCEhomeCMS.NET_ECMS_GetLastError());
            return;
        }
    ...
    public void testPause() {
        ...
        if (!CMS.hCEhomeCMS.NET_ECMS_PlayBackOperate(1, 0, struPlaybackPauseParam.getPointer())) {
            System.out.println("NET_ECMS_PlayBackOperate failed, error code:" + CMS.hCEhomeCMS.NET_ECMS_GetLastError());
            return;
        } else {
        ...
        if (playCtrl == null) {
            synchronized (PlayCtrl.class) {
                ...
                try {
                    if (osSelect.isWindows())
                        //win系统加载库路径(路径不要带中文)
                        strPlayPath = System.getProperty("user.dir") + "\\ruoyi-isup\\win-lib\\PlayCtrl.dll";
                    else if (osSelect.isLinux())
                        //Linux系统加载库路径(路径不要带中文)
                        strPlayPath = System.getProperty("user.dir") + "/ruoyi-isup/linux-lib/libPlayCtrl.so";
    ...
    public static class PLAYBACK_DATA_CB_WIN implements HCISUPStream.PLAYBACK_DATA_CB {
        ...
        public boolean invoke(int iPlayBackLinkHandle, HCISUPStream.NET_EHOME_PLAYBACK_DATA_CB_INFO pDataCBInfo, Pointer pUserData) {
            ...
            switch (pDataCBInfo.dwType) {
                case HCISUPStream.NET_EHOME_SYSHEAD: //系统头
                    // 初始化回放标记
...
//                        }
                ...
                case HCISUPStream.NET_EHOME_STREAMDATA:   //码流数据
                    ...
                    break;
                case HCISUPStream.NET_EHOME_STREAMEND:  // 视频流结束标记
