...
export default {
  ...
  methods: {
    ...
    create() {
      ...
      this.jessibucaPlayer[this._uid] = new window.Jessibuca({...options});

      let jessibuca = this.jessibucaPlayer[this._uid];
      let _this = this;
    ...
    play: function (url) {
      if (this.jessibucaPlayer[this._uid]) {
        this.destroy();
      }
      this.create();
      this.jessibucaPlayer[this._uid].on("play", () => {
        this.playing = true;
        this.loaded = true;
        this.quieting = jessibuca.quieting;
      });
      if (this.jessibucaPlayer[this._uid].hasLoaded()) {
        this.jessibucaPlayer[this._uid].play(url);
      } else {
        this.jessibucaPlayer[this._uid].on("load", () => {
          this.jessibucaPlayer[this._uid].play(url);
        });
      }
    },
    pause: function () {
      if (this.jessibucaPlayer[this._uid]) {
        this.jessibucaPlayer[this._uid].pause();
      }
      this.playing = false;
      this.err = "";
      this.performance = "";
    ...
    screenshot: function () {
      if (this.jessibucaPlayer[this._uid]) {
        this.jessibucaPlayer[this._uid].screenshot();
      }
    },
    mute: function () {
      if (this.jessibucaPlayer[this._uid]) {
        this.jessibucaPlayer[this._uid].mute();
      }
    },
    cancelMute: function () {
      if (this.jessibucaPlayer[this._uid]) {
        this.jessibucaPlayer[this._uid].cancelMute();
      }
    },
    destroy: function () {
      if (this.jessibucaPlayer[this._uid]) {
        this.jessibucaPlayer[this._uid].destroy();
      }
      if (document.getElementById("buttonsBox") == null) {
        this.$refs.container.appendChild(this.btnDom)
      }
      this.jessibucaPlayer[this._uid] = null;
      this.playing = false;
      this.err = "";
      this.performance = "";
    ...
    fullscreenSwich: function () {
      ...
      this.jessibucaPlayer[this._uid].setFullscreen(!isFull)
